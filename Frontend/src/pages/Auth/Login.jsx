...
import { Activity, Heart, User, Lock, ArrowRight } from 'lucide-react'; // Icon library

// Main Login component
const Login = () => {
  const navigate = useNavigate();     // Hook for navigating between pages
  const location = useLocation();     // Hook for accessing route state
  const { login } = useAuth();         // Custom hook to use authentication context

  // State to manage form inputs, errors, success messages, and loading state
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-purple-100 via-purple-50 to-white p-4">
      ...
      {/* Animated Card containing login form */}
      <motion.div
          ...
          <CardContent>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                ...
                >
                  {isLoading ? (
                    // Show loading animation if submitting
                    <motion.div
                      animate={{ rotate: 360 }}
