...
const Historical = () => {
  ...
  // Modify the state of the cache in order to save by metric and period
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        // Determine what metrics we need to load
        const metricsToFetch = view === 'overview' 
          ? metrics // In general view, we need all of them
          : selectedMetrics.length === 0 
            ? metrics // If there are not filters, we need all of them
            : metrics.filter(m => selectedMetrics.includes(m.id)); // Only filtered metrics 

        const results = {};
        const promises = metricsToFetch
          .filter(metric => !cachedData[selectedPeriod]?.[metric.id]) // Only fetch if not cached
          .map(metric => 
    ...
    fetchData();
  }, [user.id, selectedPeriod, view, selectedMetrics]); // We need these dependencies to determine what to load
  ...
  const getMetricStats = (metricData, metricId) => {
    ...
    // For cumulative metrics (steps, water, exercise)
    if (metricId === 'steps' || metricId === 'water' || metricId === 'exercise') {
        const totalValue = data[`${metricId}_total`];
        const daysInPeriod = metricData.length || 1; // Make sure we don't divide by 0
    ...
    // For measuring metrics (weight, muscle, fat)
  ...
  const renderDetailedMetric = (metric) => {
    ...
    if (!data[metric.id]) {
      return (
        <ChartCard
          ...
          data={[]} // We pass empty array to display no data message
          headerExtra={
    ...
    // For exercises, we used AreaChart
    ...
    // For the rest of the metrics, we keep the LineChart
    return (
      <ChartCard
        key={metric.id}
        title={metric.title}
